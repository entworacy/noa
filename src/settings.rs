...
pub struct IrisHookConfig {
    ...
    pub endpoint_bridge_url: String,
...
impl IrisHookConfig {
    pub async fn publish(&self) -> std::io::Result<()> {
        ...
        let payload = serde_json::to_vec_pretty(&IrisHookFile {
            ...
            endpoint_bridge_url: &self.endpoint_bridge_url,
...
impl Settings {
    pub fn from_env() -> Self {
        ...
        let iris_hook = IrisHookConfig {
            ...
            endpoint_bridge_url: non_empty_env("NOA_IRIS_ENDPOINT_BRIDGE_URL").unwrap_or_else(
                || format!("http://127.0.0.1:{bridge_port}/internal/iris/endpoint"),
            ),
            endpoint_prefix: iris_endpoint_prefix(),
        ...
        Self {
            ...
            poll_interval_ms: parse_env("NOA_POLL_INTERVAL_MS", 30_000_u64).max(1_000),
            snapshot_interval_ms: parse_env("NOA_SNAPSHOT_INTERVAL_MS", 60_000_u64).max(5_000),
...
fn iris_endpoint_prefix() -> String {
    let requested = non_empty_env("NOA_IRIS_ENDPOINT_PREFIX").unwrap_or_else(|| "/noa".to_string());
    let normalized = format!("/{}", requested.trim_matches('/'));
    if normalized == "/"
        || normalized.contains(['{', '}', '*', '?', '#'])
...
mod tests {
    ...
    async fn publishes_private_iris_hook_configuration() {
        ...
        assert_eq!(
            json["endpointBridgeUrl"],
            "http://127.0.0.1:4000/internal/iris/endpoint"
        );
        assert_eq!(json["endpointPrefix"], "/noa");
