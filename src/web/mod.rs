...
pub fn configure(config: &mut web::ServiceConfig) {
    config
        ...
        .route(
            "/api/rooms/{chat_id}/messages/{log_id}/hide",
...
pub(super) async fn hide_message_action(
    chat_id: String,
    log_id: String,
    state: &AppState,
) -> Result<serde_json::Value, NoaError> {
    let chat_id = parse_chat_id(&chat_id)?;
    let log_id = parse_positive_i64("logId", &log_id)?;
    if !state.config.kakao_hook_enabled {
        return Err(NoaError::AndroidUnavailable(
            "메시지 가리기는 KakaoTalk 후킹 모드가 필요합니다".to_string(),
        ));
    }
    let catalog = state.catalog.clone().ok_or_else(|| {
        NoaError::Database(
            "가리기 대상 메시지를 검증할 KakaoTalk 데이터베이스가 없습니다".to_string(),
        )
    })?;
    let target_catalog = catalog.clone();
    let target =
        tokio::task::spawn_blocking(move || target_catalog.hide_message_target(chat_id, log_id))
            .await
            .map_err(|error| {
                NoaError::Internal(format!("가리기 대상 조회 작업 실패: {error}"))
            })??;
    let message_type = i32::try_from(target.message_type).map_err(|_| {
        NoaError::Database(format!(
            "메시지 타입 범위가 올바르지 않습니다: {}",
            target.message_type
        ))
    })?;
    crate::intercept::hide_message(chat_id, log_id, message_type, target.message).await?;
    Ok(serde_json::json!({
        "ok": true,
        "accepted": true,
        "verified": false,
...
fn parse_positive_i64(name: &str, value: &str) -> Result<i64, NoaError> {
    value
        .parse::<i64>()
        .ok()
        .filter(|value| *value > 0)
        .ok_or_else(|| NoaError::BadRequest(format!("올바르지 않은 {name}: {value}")))
}
    ...
    async fn serves_dashboard_and_enforces_api_token() {
        ...
        assert!(
            endpoint_index["endpoints"]
                ...
                .unwrap()
                .iter()
                .any(|value| value == "POST /rooms/{chatId}/messages/{logId}/hide")
        );
        assert!(
            endpoint_index["endpoints"]
                .as_array()
        ...
        let endpoint_hide_invalid_log = test::call_service(
            &app,
            test::TestRequest::post()
                .uri("/internal/iris/endpoint/rooms/123/messages/not-a-number/hide")
                .insert_header(("X-Noa-Hook-Token", "hook-token"))
                .to_request(),
        )
        .await;
        assert_eq!(endpoint_hide_invalid_log.status(), StatusCode::BAD_REQUEST);
